...
─────────────────────────────────────────────────────────────────────────
P0 🔴     输出侧文件路径可点击    @file 可点击跳转      ❌ 缺失     2-3 天
P0 🔴     Context Chip 组件     输入框上方引用标签     ❌ 缺失     2-3 天
P0 🔴     行号跳转              @file:line 跳转       ❌ 缺失     1-2 天

P1 🟡     文件跳转机制          桌面/终端打开文件      ❌ 缺失     1-2 天
P1 🟡     选中代码引用 UI        代码选中→浮动菜单     ❌ 缺失     2-3 天
P1 🟡     历史消息引用 UI        消息 hover→引用       ❌ 缺失     2-3 天

P2 🟢     代码块来源标注         代码块上方文件路径    ❌ 缺失     1 天
P2 🟢     引用样式优化          高亮 + 视觉区分        ❌ 缺失     1 天
P2 🟢     引用 Token 统计        显示引用消耗          ❌ 缺失     0.5 天
```
